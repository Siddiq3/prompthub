...
export default function CategoryPromptBrowser({ category, prompts }: CategoryPromptBrowserProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 to-slate-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-8">
        <nav aria-label="Breadcrumb" className="mb-4 text-sm text-slate-400">
          <ol className="flex flex-wrap items-center gap-2">
            ...
            <li className="text-white font-semibold">{category}</li>
          </ol>
        </nav>

        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            <h1 className="text-[56px] leading-[0.96] tracking-[-0.03em] font-black text-white sm:text-[64px] md:text-[72px]" style={{ fontFamily: 'Clash Display, ui-sans-serif, system-ui' }}>
              {category}
            </h1>
            <p className="mt-4 text-sm text-slate-400">{formatCountLabel(totalCount, category)}</p>
          </div>

          <div className="flex flex-wrap gap-3 items-center">
            <div className="rounded-full bg-slate-900/70 px-4 py-2 text-sm text-slate-300 ring-1 ring-white/10">
              {filteredCount === totalCount
                ? `Showing ${filteredCount} prompts`
                : `Showing ${filteredCount} of ${totalCount} prompts`}
            </div>
            <div className="hidden md:inline-flex items-center gap-2 rounded-full border border-white/10 bg-[#0B0E1A]/90 px-4 py-2 text-sm text-slate-300">
      ...
      <div className="sticky top-16 z-30 border-b border-white/10 bg-[#0B0E1A] bg-opacity-95 backdrop-blur-xl shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="flex flex-wrap items-center gap-3">
              <button
                type="button"
                className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-slate-950/90 px-4 py-2 text-sm font-semibold text-white transition hover:bg-slate-900 md:hidden"
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="hidden md:flex items-center gap-2 text-sm text-slate-400">
                ...
                <span className="inline-flex h-6 min-w-[1.5rem] items-center justify-center rounded-full bg-white/10 px-2 text-xs font-semibold text-white">
                  {activeFilters}
                </span>
              </div>
              ...
              <select
                id="category-sort"
                className="h-11 rounded-full border border-white/10 bg-slate-950/90 px-4 py-3 text-sm text-white outline-none transition focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20"
                value={sortBy}
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2 rounded-3xl border border-white/10 bg-slate-950/80 px-4 py-3 text-sm text-slate-300 md:hidden">
            {activeFilters === 0 ? (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        {filteredCount === 0 ? (
          <div className="rounded-[28px] border border-white/10 bg-slate-950/80 p-12 text-center shadow-[0_24px_80px_rgba(15,23,42,0.45)]">
            <div className="mx-auto mb-6 flex h-24 w-24 items-center justify-center rounded-3xl bg-gradient-to-br from-violet-500/20 to-slate-900 text-violet-300">
            ...
            <h2 className="text-2xl font-semibold text-white">No prompts match these filters</h2>
...
function FilterGroup({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-3 rounded-3xl bg-white/5 p-3 text-sm text-slate-300 shadow-sm ring-1 ring-white/5">
