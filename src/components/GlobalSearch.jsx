                        ...
                        {results.prompts.map((p, idx) => {
                          ...
                          return (
                            <li key={p.id} id={`prompt-item-${idx}`} role="option" aria-selected={active} className={`flex items-center gap-3 rounded-md px-3 py-2 hover:bg-white/5 ${active ? 'ring-2 ring-violet-500/60 bg-white/5' : ''}`}>
                              {p.previewImage || p.image ? (
                              <div className="relative h-10 w-10 flex-shrink-0 overflow-hidden rounded-md">
                                <Image
                                  src={p.previewImage || p.image}
                                  alt={p.title || p.category || 'Prompt preview'}
                                  fill
                                  className="object-cover"
                                />
                              </div>
                            ) : (
                              <div className="flex h-10 w-10 items-center justify-center rounded-md bg-slate-700 text-xs text-slate-200 flex-shrink-0">
                                AI
                              </div>
                            )}
